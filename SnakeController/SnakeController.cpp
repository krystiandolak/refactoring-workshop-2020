...
void Controller::isGameLost(Segment& head, bool& isLost) {
...
void Controller::isHeadOnFruit() {
    m_scorePort.send(std::make_unique<EventT<ScoreInd>>());
    m_foodPort.send(std::make_unique<EventT<FoodReq>>());
}

void Controller::snakeOutOfMap(bool& isLost) {
    m_scorePort.send(std::make_unique<EventT<LooseInd>>());
    isLost = true;
}

void Controller::setFreeLastSegment() {
    for (auto &segment : m_segments) {
        if (not --segment.ttl) {
            DisplayInd l_evt;
            l_evt.x = segment.x;
...
void Controller::nextMoveSnake(Segment& head) {
...
void Controller::receive(std::unique_ptr<Event> e)
{
    try {
        ...
        isGameLost(newHead, lost);

        bool isSnakeOutOfMap = (newHead.x < 0 or newHead.y < 0 or
